 ...
 * json和redis中存储的字段名称和含�?
 * k		对称加密的密�?
 * en_t    对称加密算法的类型，0:表示不加密，1：表示简单混淆加�?
 * st       连接的状态：1：在线，2：不在线�?：刚刚获取mosq server
 * hi		该连接所在的mosquitto的内部地�?��mosquitto要被内网和外网同时访问）
 * pi       该连接所在的mosquitto的内部端口号
 * ho		该连接所在的mosquitto的外部地�?
 * po		该连接所在的mosquitto的外部端口号
 * ss       连接的会话，客户端连接mosquitto的时候使�?
 * mf       mosquittoserver的标识，例如可以为mosquitto的内部地�?端口�?
 */
public class ConnectionInfo {
    ...
    /**
     * 从一个map中来初始化这个连接对象，map的key也必须遵循本类的定义，否则无法解�?
     * */
    ...
    /**
     * 获取�?��connid的详细信息，不包括connid，这个map可以直接用于redis存储
     * */
    ...
    /*
    *外部�?��版的信息，不包括hi、pi这些数据，主要用于返回给客户端使�?
    * */
